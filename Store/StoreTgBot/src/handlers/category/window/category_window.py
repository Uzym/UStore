...
async def get_category_photos(manager: DialogManager, **kwargs):
...
async def category_getter(manager: DialogManager, **kwargs):
    category_id = int(manager.start_data['category_id'])
    category_data = await category_service.get_category(category_id=category_id)
    photos = await get_category_photos(manager)
    return {
...
async def get_category_button(callback: CallbackQuery, widget: Any, manager: DialogManager, item_id: str):
    await callback.answer(text=LEXICON["loading"])
    manager.start_data["category_id"] = item_id
    manager.dialog_data["category_id"] = item_id
    await manager.switch_to(Category.category)


async def to_category_update_button(callback: CallbackQuery, button: Button, manager: DialogManager, **kwargs):
...
async def add_category_photo_button(callback: CallbackQuery, button: Button, manager: DialogManager, **kwargs):
    await manager.switch_to(Category.category_wait_photo)
...
async def add_category_photo(message: Message, message_input: MessageInput, manager: DialogManager):
    file_id = message.photo[-1].file_id
    file_info = await bot.get_file(file_id)
    file = await bot.download_file(file_info.file_path)
    photo_data = await s3_service.upload_file(file_path=file_info.file_path, file=file)
    res = await photo_service.create_photo(name=photo_data.fileName, category_id=manager.dialog_data['category_id'])
    await manager.switch_to(Category.category)
...
async def category_photos_delete_button(callback: CallbackQuery, button: Button, manager: DialogManager, **kwargs):
    await manager.switch_to(Category.category_delete_photo)
...
async def delete_category_photo_button(callback: CallbackQuery, widget: Any, manager: DialogManager, item_id: str):
